...
namespace dehancer::network::client {
    ...
    namespace detail {
        ...
        struct Response {
            ...
            rx::subjects::subject<std::string> body_progress;
    ...
    rx::observable<std::shared_ptr<HttpResponse>> UrlSession::request(const HttpRequest &request) const {
      return rx::observable<>::create<std::shared_ptr<HttpResponse>>(
              ...
              [this, &request](rx::subscriber<std::shared_ptr<HttpResponse>>& subscriber) {
                          ...
                          .get_observable().subscribe(
                                  [&response, curl, &subscriber](std::string chunk) {
                                      try {
                                        ...
                                        std::cmatch what;
                                        if (std::regex_match(chunk.c_str(), chunk.c_str() + chunk.size()-2, what, detail::__http_header_regex_)) {
                                          if (what.size() == 3 ) {
                  ...
                  curl_response
                          .body_progress
                          .get_observable().subscribe(
                                  [&response, &subscriber](const std::string &chunk) {
                                      response->body.append(chunk);
                                      if (response->content_length>0) {
                                        float p = float(response->body.size())/float(response->content_length)*100.0;
                  ...
                  curl_easy_setopt(curl, CURLOPT_URL, session_->url.c_str());
                  ...
                  curl_easy_setopt(curl, CURLOPT_TIMEOUT, session_->timeout);
                  ...
                  ///
                  if (res != CURLE_OK) {
                    rx::observable<>::error<std::shared_ptr<HttpResponse>>(exception(res)).subscribe(subscriber);
                  }

                  else if (response->http_status>=400) {
                    rx::observable<>
                    ::error<std::shared_ptr<HttpResponse>>(exception("Http response error...", response))
                            .subscribe(subscriber);
                  }
